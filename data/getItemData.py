...
def getItemDetail(nomor):
    ...
    df = getResult(cursor, f"""
            SELECT 
                a.NO_MR_SR, MRSR.TGL_MRSR, 
                a.NO_PR, PR.TGL_PR,
                a.NO_PO, PO.TGL_PO,
                a.URAIAN, a.NAMAVENDOR
                FROM 
                    (SELECT 
                        NO_MR_SR, a.NO_PR AS NO_PR, b.PO_NO AS NO_PO, 
                        URAIAN, NAMAVENDOR, COMP_CODE
                    FROM (
                        SELECT x.NO_MR_SR AS NO_MR_SR, COMP_CODE, NO_PR, x.URAIAN
...
def getVendorOption(cursor):
    return getResult(cursor, f"""SELECT DISTINCT(NAMA) AS m FROM SIMONA.PUBLIC.MS_VENDOR WHERE COMP_CODE = 'J000';""")
